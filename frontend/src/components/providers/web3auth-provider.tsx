...
export function Web3AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // v9 supports configureAdapter — popup mode avoids redirect/WalletServices issues
  ...
  // Opens the Web3Auth modal — user picks Google / Email / Wallet inside
  const login = useCallback(async (): Promise<IProvider | null> => {
    ...
    if (p) { setProvider(p); setIsConnected(true); }
    return p ?? null;
  }, []);
